...
def svm_loss_vectorized(
    ...
    num_classes = W.shape[1]
    num_train = X.shape[0]
    score = torch.mm(X, W) # 128 * 10
    correct_class_score = score[range(num_train),y].view(-1,1) # 128 * 1
    margin = torch.maximum(torch.zeros(num_classes, dtype=X.dtype, device=X.device), score - correct_class_score + 1)
    margin[range(num_train),y] = 0 # remove the yi
    ...
    #############################################################################
    # Replace "pass" statement with your code
    # is equal to for loop j in range(number of classes)
    margin[margin>0] = 1
    number_of_margin_counts = margin.sum(axis=1) # each j loop, how many margin >0
    margin[range(num_train),y] -= number_of_margin_counts # how many Syi for gradient calculate 
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    indexs = torch.randint(num_train,(batch_size,))
    X_batch = X[indexs]
    y_batch = y[indexs]
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [7e-3, 3e-2, 2e-2, 1e-2]
    regularization_strengths = [2e-3, 1e-3, 3e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        for j, s in enumerate(score):
            # sum the denominator of softmax function
            denominator += torch.exp(s)
            if j == y[i]:
                numerator = torch.exp(s)
        # dW can only be calculated after we have the denominator 
        for j, s in enumerate(score):
            if j == y[i]:
                dW[:,y[i]] +=  -1  * X[i] # i = j
            dW[:,j] += (torch.exp(s)/denominator) * X[i] # i!=j 
...
def softmax_loss_vectorized(
    ...
    score = torch.mm(X,W)
    # ensure numeric stability 
    score -= torch.max(score,dim=1)[0].view(-1,1)
    # loss 
    numerator= torch.exp(score[range(num_train),y]) 
    denominator = torch.sum(torch.exp(score),1)
    loss = torch.sum(-1 * torch.log(numerator/denominator))
    # dw
    # consider i!=j and i=j 
    correct_class = torch.zeros((num_train,num_classes),dtype=X.dtype,device=X.device)
    # the difference is -1 * X
    correct_class[range(num_train),y] = -1
    dW = torch.mm(X.t()/denominator, torch.exp(score)) + torch.mm(X.t(), correct_class)
    loss /= num_train
    dW /= num_train
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1.48e-1,1.5e-1,1.47e-1,1.49e-1]
    regularization_strengths = [1e-5,1e-4,1e-3,1e-6]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
