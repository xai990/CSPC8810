...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    # first layer score 
    activation = torch.mm(X,W1) + b1 # N*H
    # Relu function for score < 0
    Relu = torch.zeros((activation.shape[0], activation.shape[1]),dtype=X.dtype,device=X.device)
    hidden = torch.maximum(activation, Relu)
...
def nn_forward_backward(
    ...
    scores -= torch.max(scores,dim=1)[0].view(-1,1) # N*C
    numerator= torch.exp(scores[range(N),y])
    denominator = torch.sum(torch.exp(scores),1)
    softmax = numerator / denominator
    loss = torch.sum(-1 * torch.log(softmax))
    ...
    # Replace "pass" statement with your code
    # top layer
    # dout/dw2 = dout/de * de/ds * ds/dw2
    dout = torch.exp(scores)/ torch.sum(torch.exp(scores),dim=1,keepdim=True) # N*C
    dout[range(N),y] -= 1 # i=j derivatative = (pi-1)pj
    dW2 = torch.mm(h1.t(), dout) # H*N * N*C
    db2 = torch.sum(dout,0)
    # hidden relu 
    
    dh1 = torch.mm(dout, W2.t()) # N*C * C*H  
    # <0 derivative =0
    dh1[h1==0] = 0
    # dw1
    #dout/dw1  = dout/dfront * dfront/dw1
    dW1 = torch.mm(X.t(), dh1) # C*N * N*H
    db1 = torch.sum(dh1,0)
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    activation = torch.mm(X,params['W1']) + params['b1']
    Relu = torch.zeros((activation.shape[0], activation.shape[1]),dtype=X.dtype,device=X.device)
    hidden = torch.maximum(activation, Relu)
    score = torch.mm(hidden, params['W2']) + params['b2']
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [5e-1, 1, 1.5]
    hidden_sizes = [ 128,256]
    regularization_strengths = [1e-7,1e-5,1e-6]
            ...
            for reg in regularization_strengths:
                for lrd in learning_rate_decays:
                    ...
                    if stats['val_acc_history'][-1] > best_val_acc:
                        print('Training two layer net with learning_rate=%e, hidden_size=%e, reg=%e, learning_rate_decays=%e'
                                  % (lr, hs, reg, lrd))
                        best_net = net
                        best_stat = stats
