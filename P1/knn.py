...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        dists[i] = torch.sum(torch.square(x_train[i] - x_test), dim=1).t()
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train = x_train.view(num_train, -1)
    x_test = x_test.view(num_test, -1)
    dists = torch.sum(x_train**2, dim=1, keepdim=True)+torch.sum(x_test **2, dim=1, keepdim=True).t() - 2* torch.mm(x_train,x_test.t())
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        labels, counts = torch.unique(y_train[indices],return_counts=True)
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x_test_f = x_train_folds[i]
            y_test_f = y_train_folds[i]
            x_train_f_r = torch.cat((*x_train_folds[:i],*x_train_folds[i+1:]),0)
            y_train_f_r = torch.cat((*y_train_folds[:i],*y_train_folds[i+1:]),0)
            
            classifier = KnnClassifier(x_train_f_r, y_train_f_r)
            accuracy = classifier.check_accuracy(x_test_f,y_test_f,k)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accuracy in k_to_accuracies.items():
        means[k] = statistics.mean(accuracy)
