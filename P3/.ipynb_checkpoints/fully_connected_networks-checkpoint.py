...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, cache = Linear.forward(X2,W2,b2)
        ...
        dx2, dw2, db2 = Linear.backward(dout, cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            self.params[f'W{i+1}'] = weight_scale * torch.randn(dims[i], dims[i+1], device=device, dtype = dtype)
            self.params[f'b{i+1}'] = torch.zeros(dims[i+1],device=device, dtype = dtype)
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict,optim_config={
            ...
            lr_decay=0.95,
            num_epochs=50, 
            batch_size=100, print_every=100, device ='cuda')
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand(*x.shape,device = x.device, dtype = x.dtype) < p) / p
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
