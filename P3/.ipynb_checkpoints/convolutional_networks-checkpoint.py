...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out = torch.zeros (( N,F,H_next,W_next), device=x.device, dtype=x.dtype)
        x_pad = torch.nn.functional.pad(x,(pad,pad,pad,pad)).to(x.device).to(x.dtype)
        
        for n in range(N):
            for f in range(F):
                for hh in range(0,H,stride):
                    for ww in range(0, W, stride):
                        out[n,f,hh//stride,ww//stride] = (w[f]*x_pad[n, :, hh:hh+HH, ww:ww+WW]).sum() + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
            for f in range(F):
                for hh in range(0,H,stride):
                    for ww in range(0,W,stride):
                        dw[f] += x_pad[n,:,hh:hh+HH,ww:ww+WW] * dout[n,f,hh//stride,ww//stride]
                        dx_pad[n,:,hh:hh+HH,ww:ww+WW] += w[f] * dout[n,f,hh//stride,ww//stride]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        H_next = 1 + (H - pool_height) // stride 
        W_next = 1 + (W - pool_width) // stride 
        out = torch.zeros((N,C,H_next,W_next),dtype=x.dtype,device=x.device)
        for n in range(N):
            for c in range(C):
                for hh in range(0,H,stride):
                    for ww in range(0,W,stride):
                        out[n,c,hh//stride,ww//stride] = torch.max(x[n,c,hh:hh+pool_height,ww:ww+pool_width])
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
            for c in range(C):
                for hh in range(0,H,stride):
                    for ww in range(0,W,stride):
                        idexs = torch.argmax(x[n,c,hh:hh+pool_height,ww:ww+pool_width])
                        idexs_x = idexs // pool_height
                        idexs_y = idexs % pool_width
                        
                        dx[n,c,hh:hh+pool_height,ww:ww+pool_width][idexs_x,idexs_y] = dout[n,c,hh//stride,ww//stride]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W2'] = weight_scale * torch.randn(num_filters * H * W // 4, hidden_dim, dtype=dtype,device=device)
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        X1, Conv_ReLU_Pool_cache = Conv_ReLU_Pool.forward(X, W1,b1,conv_param,pool_param)
        X2, Linear_ReLU_cache =Linear_ReLU.forward(X1,W2,b2)
        X3, Linear_cache = Linear.forward(X2,W3,b3)
        scores = X3
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dout,grads['W3'],grads['b3'] = Linear.backward(dout,Linear_cache)
        grads['W3'] += 2 * self.reg * self.params['W3']
        dout,grads['W2'],grads['b2'] = Linear_ReLU.backward(dout,Linear_ReLU_cache)
        grads['W2'] += 2 * self.reg * self.params['W2']
        _, grads['W1'],grads['b1'] = Conv_ReLU_Pool.backward(dout,Conv_ReLU_Pool_cache)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(len(num_filters)):         
            if i != 0:
                ...
                if weight_scale == "kaiming":
                    self.params[f'W{i+1}'] = kaiming_initializer(num_filters[i],num_filters[i-1],K=filter_size,relu=True,dtype=dtype,device=device)
                else:
                    self.params[f'W{i+1}'] = weight_scale * torch.randn(num_filters[i],num_filters[i-1],filter_size,filter_size,dtype=dtype,device=device)
                self.params[f'b{i+1}'] = torch.zeros(num_filters[i],dtype=dtype,device=device)
            else:
                H_out = 1 + (H + 2 * conv_param['pad'] - filter_size) // conv_param['stride']
                W_out = 1 + (W + 2 * conv_param['pad'] - filter_size) // conv_param['stride']
                if weight_scale == "kaiming":
                    self.params[f'W{i+1}'] = kaiming_initializer(num_filters[i],C,K=filter_size,relu=True,dtype=dtype,device=device)
                else:
                    self.params[f'W{i+1}'] = weight_scale * torch.randn(num_filters[i],C,filter_size,filter_size,dtype=dtype,device=device)
                self.params[f'b{i+1}'] = torch.zeros(num_filters[i],dtype=dtype,device=device)
        ...
        i+=1
        if weight_scale == "kaiming":
            self.params[f'W{i+1}'] = kaiming_initializer(num_filters[i-1]*H_out*W_out,num_classes,relu=True,dtype=dtype,device=device)
        else:
            self.params[f'W{i+1}'] = weight_scale * torch.randn(num_filters[i-1]*H_out*W_out,num_classes,dtype=dtype,device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1): 
            if i in self.max_pools:
                if self.batchnorm:
                    scores, cache[f'{i}'] = Conv_BatchNorm_ReLU_Pool.forward(scores,self.params[f'W{i+1}'], self.params[f'b{i+1}'],self.params[f'gamma{i+1}'], self.params[f'beta{i+1}'],conv_param,self.bn_params[i],pool_param)
                else:
                    scores, cache[f'{i}'] = Conv_ReLU_Pool.forward(scores,self.params[f'W{i+1}'], self.params[f'b{i+1}'],conv_param,pool_param)
            else:
                if self.batchnorm:
                    scores, cache[f'{i}']= Conv_BatchNorm_ReLU.forward(scores,self.params[f'W{i+1}'], self.params[f'b{i+1}'],self.params[f'gamma{i+1}'], self.params[f'beta{i+1}'],conv_param,self.bn_params[i])
                else:
                    scores, cache[f'{i}']= Conv_ReLU.forward(scores,self.params[f'W{i+1}'], self.params[f'b{i+1}'],conv_param)
        i += 1
        scores, cache[f'{i}'] = Linear.forward(scores,self.params[f'W{i+1}'], self.params[f'b{i+1}'])
        ...
        for i in range(self.num_layers):
            if self.batchnorm:
                if i < (self.num_layers-1):
                    loss += self.reg * torch.sum(self.params[f'gamma{i+1}']**2)
        ...
        for i in range(i-1, -1,-1):
            if i in self.max_pools:
                if self.batchnorm:
                    ...
                    grads[f'W{i+1}'] = dw + 2* self.reg*self.params[f'W{i+1}']
                    grads[f'b{i+1}'] = db
                    grads[f'beta{i+1}'] = dbetta1
                    grads[f'gamma{i+1}'] = dgamma1 + 2 * self.reg * self.params[f'gamma{i+1}']
                else:
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict,
                print_every=20, num_epochs=15, batch_size=128,
                update_rule=adam,
                optim_config={
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            ##################################################################
            # Replace "pass" statement with your code
            mu = 1. / N * torch.sum(x, 0)
            running_mean = momentum * running_mean + (1-momentum)*mu
            
            xmu = x - mu
            sqxm = xmu ** 2
            var = torch.sum(sqxm,0)/(N-1)
            running_var = momentum * running_var + (1-momentum)*var
            
            svar = torch.sqrt(var+eps)
            ivar = 1./svar
            x_hat = xmu * ivar
            gammax = gamma * x_hat 
            out = gammax + beta
            cache = (x_hat, gamma, xmu, ivar,svar,var,eps)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        x_hat, gamma, xmu, ivar,svar,var,eps = cache
        N, D = dout.shape
        dbeta = torch.sum(dout,0)
        dgamma = torch.sum(dout * x_hat,0)
        dx_hat = dout * gamma
        divar = torch.sum(dx_hat * xmu, 0)
        dxm1 = dx_hat * ivar
        dsvar = -divar/svar**2
        dvar = 0.5*dsvar/torch.sqrt(var+eps)
        dsqxm = torch.ones_like(dout)*dvar/(N-1)
        dxm2 = 2*xmu*dsqxm
        dx1 = dxm1 + dxm2
        dmu = -torch.sum(dx1,0)
    ...
    def backward_alt(dout, cache):
        ...
        dbeta = torch.sum(dout,0)
        dgamma = torch.sum(dout * x_hat,0)
        dx = gamma * ((dout - torch.sum(dout,0)/N)/svar - xmu*torch.sum(dout*xmu,0)/(svar**3*(N-1)))
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N,C,H,W = x.shape
        tweak_x = x.contiguous().transpose(0, 1).contiguous().view(C, -1).T
        tweak_out, cache = BatchNorm.forward(tweak_x, gamma,beta,bn_param)
        out = tweak_out.contiguous().T.view(C, N, H, W).transpose(0,1).contiguous()
        # N, C, H, W = x.shape
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N,C,H,W=dout.shape
        tweak_dout = dout.contiguous().transpose(0,1).contiguous().view(C,-1).T
        tweak_dx,dgamma,dbeta = BatchNorm.backward_alt(tweak_dout,cache)
        dx = tweak_dx.contiguous().T.view(C, N, H, W).transpose(0,1).contiguous()
        # N, C, H, W = dout.shape
